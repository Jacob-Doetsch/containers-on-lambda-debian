...
if [ -z "${AWS_LAMBDA_RUNTIME_API}" ]; then
  ...
  exec /usr/local/bin/aws-lambda-rie ./node_modules/.bin/aws-lambda-ric $1
else
  # We know the image is being run on Lambda so we don't need to ues the RIE.
  exec $(pwd)/node_modules/.bin/aws-lambda-ric $1
fi     
